	...
	public void run() {
		...
		if (header_parts[1].trim().equals("1.0")) {
			...
			case "PUTCHUNK":
				...
				if (message.getBody().length + ConfigManager.getConfigManager().getUsedSpace() <= ConfigManager
						.getConfigManager().getMaxSpace()) {
					...
					int wantedReplication = Integer.parseInt(header_parts[5].trim());
					...
					if (chunkOBJ == null) {
						...
						synchronized (MCListener.getInstance().pendingChunks) {
							MCListener.getInstance().pendingChunks.add(actualChunk);
						...
						// if the chunk didnt get enough stored, then im storing
						// it

						if (actualChunk.getCurrentReplicationDegree() < actualChunk.getWantedReplicationDegree()) {
							System.out.println("saving " + actualChunk.getChunkNo() + " chunk with current "
									+ actualChunk.getCurrentReplicationDegree());
							actualChunk.setSize(message.getBody().length);
							ChunkBackup.getInstance().storeChunk(actualChunk, message.getBody());
							
							// verify after some time if the chunk has the replication degree desired
	                        new Thread(new Runnable() {
	                            ...
	                            public void run() {
	                                while (true) {
	                                    ...
	                                    Chunk chunk = ConfigManager.getConfigManager().getSavedChunk(fileID, chunkNo);
						...
						// remove the temporary chunk from the pending chunks list
						synchronized (MCListener.getInstance().pendingChunks) {
							MCListener.getInstance().pendingChunks.remove(actualChunk);
						}
					}
				} else {
					System.out.println(("body " + message.getBody().length + " used "
							+ ConfigManager.getConfigManager().getUsedSpace() + " max "
							+ ConfigManager.getConfigManager().getMaxSpace()));
			...
			System.out.println("MDB: message with different version");
	}
}
